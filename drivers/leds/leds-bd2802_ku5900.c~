...
struct i2c_client *bb2802_i2c_client;//2011205 kyungyoon.kim@lge.com lcd resume speed
...
#define BD2802_CURRENT_BLUE_MIN			0x00 /* 1mA */
...
// LGE_B_DOM_S  20101230 nttaejun.cho@lge.com, RGB Led
u8 key_led_flag=0;
// LGE_B_DOM_E  20101230 nttaejun.cho@lge.com, RGB Led
...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2012.06.05] - Debug log disabled.
	//printk("############## system_rev = %d\n", system_rev);
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2012.06.05]- Debug log disabled.
...
static void bd2802_turn_white(struct bd2802_led *led, enum key_leds id)
{
	...
	DBG("id=%d,led->white_current=0x%x\n",id,led->white_current);
...
static void bd2802_turn_blue(struct bd2802_led *led, enum key_leds id)
{
	...
	DBG("id=%d,led->blue_current=0x%x\n",id,led->blue_current);
...
EXPORT_SYMBOL(bd2802_touch_on);

void bd2802_touch_timer_restart()
{
  	struct bd2802_led *led = i2c_get_clientdata(bd2802_i2c_client);
...
EXPORT_SYMBOL(bd2802_touch_timer_restart);
// LGE_B_DOM_E  20101230 nttaejun.cho@lge.com, RGB Led
...
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.09.09]- Merge from Black_Froyo MR Ver.
	...
	led->blue_current = BD2802_CURRENT_000;
// LGE_B_DOM_S 20101230 ntjinsu.park@lge.com, RGB Led
#if 1	
    led->led_state=BD2802_ON;
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.09.09] - Merge from Black_Froyo MR Ver.
    //bd2802_off(led);
...
static struct device_attribute bd2802_blink_enable_attr = {
	.attr = {
		.name = "blink_enable",
		.mode = 0666,
//		.owner = THIS_MODULE	//Build error in ICS
	},
...
// LGE_B_DOM_S 20110106 nttaejun.cho@lge.com, Justin LED onoff			

static ssize_t bd2802_show_led_onoff(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct bd2802_led *led = i2c_get_clientdata(to_i2c_client(dev));
...
// LGE_B_DOM_E 20110106 nttaejun.cho@lge.com, Justin LED onoff
#define BD2802_VALUE_ON		255
#define BD2802_VALUE_OFF	0
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((value==BD2802_VALUE_ON)/*&&(led->led_state!=BD2802_ON)*/)
	{
		led->led_state = BD2802_ON;
		led->white_current = BD2802_CURRENT_WHITE_MAX;
	    led->blue_current = BD2802_CURRENT_000;
	...
	else if ((value==BD2802_VALUE_OFF)/*&&(led->led_state!=BD2802_OFF)*/)
	{
		bd2802_off(led);
		bd2802_reset(led);
	...
	{
	    if ((value != BD2802_VALUE_ON) && (value != BD2802_VALUE_OFF))
	    {
			DBG("Value(%d) is not valid\n", value);
			return -EINVAL;
		}
	}
...
static struct device_attribute bd2802_reg##reg_addr##_attr = {		\
	.attr = {.name = reg_name, .mode = 0666, /* .owner = THIS_MODULE // build error in ICS */},	\
...
void bd2802_resume_for_lcd(void)
{
	struct bd2802_led *led = i2c_get_clientdata(bd2802_i2c_client);
	DBG("\n");
...
EXPORT_SYMBOL(bd2802_resume_for_lcd);
			...
			const struct i2c_device_id *id)
{
	...
	if (!led->bd2802_wq)
		return -ENOMEM;
/* dajin.kim@lge.com - use one workqueue in bd2802
...
#endif /* 20110304 seven.kim@lge.com late_resume_lcd [START] */
	.id_table	= bd2802_id,
};
...
{	
	...
	ret = i2c_add_driver(&bd2802_i2c_driver);
